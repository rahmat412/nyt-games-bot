...
class StrandsCommandHandler(BaseCommandHandler):
    ...
    async def get_entry(self, ctx: commands.Context, *args: str) -> None:
        ...
        if user_id in self.db.get_all_players():
            user_puzzles: list[StrandsPuzzleEntry] = self.db.get_entries_by_player(
                user_id
            )
            df = pd.DataFrame(
                columns=["User", "Puzzle #", "Rating", "Hints", "🟡 Index", "Puzzle"]
            )
    ...
    async def get_stats(self, ctx: commands.Context, *args: str) -> None:
        ...
        df = pd.DataFrame(
            columns=["User", "Avg Rating", "Avg Hints", "Avg 🟡 Index", "🧩", "🚫"]
        )
        ...
        if len(user_ids) < 5:
            ...
            for i, user_id in enumerate(user_ids):
                ...
                entries: list[StrandsPuzzleEntry] = self.db.get_entries_by_player(
                    user_id
                )
    ...
    async def add_score(self, ctx: commands.Context, *args: str) -> None:
        if args is not None and len(args) >= 3:
            ...
            await ctx.reply(
                "To manually add a Strands score, please use `?add <user> <Strands output>` (specifying a user is optional)."
            )

...
class ConnectionsCommandHandler(BaseCommandHandler):
    ...
    async def get_entry(self, ctx: commands.Context, *args: str) -> None:
        ...
        if user_id in self.db.get_all_players():
            user_puzzles: list[ConnectionsPuzzleEntry] = self.db.get_entries_by_player(
                user_id
            )
    ...
    async def get_stats(self, ctx: commands.Context, *args: str) -> None:
        ...
        if len(user_ids) < 5:
            ...
            for i, user_id in enumerate(user_ids):
                ...
                entries: list[ConnectionsPuzzleEntry] = self.db.get_entries_by_player(
                    user_id
                )
    ...
    async def add_score(self, ctx: commands.Context, *args: str) -> None:
        if args is not None and len(args) >= 4:
            ...
            await ctx.reply(
                "To manually add a Connections score, please use `?add <user> <Connections output>` (specifying a user is optional)."
            )
